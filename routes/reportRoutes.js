...
router.post('/', async(req,res) => {
    try{
        const{ description, longitude, latitude, before_photo_url, after_photo_url } = req.body;
        ...
        const newReport = await pool.query(`insert into reports (description, location, ward_id, before_photo_url, after_photo_url, status, created_at, updated_at)
            values ($1,ST_SetSRID(ST_MAKEPOINT($2,$3), 4326), (SELECT ward_id fROM wards WHERE ST_Contains(boundaries,ST_SetSRID(ST_MakePoint($2, $3), 4326)))
            ,$4, $5,'Pending', NOW(), NOW()) RETURNING *`, 
        [description, longitude, latitude, before_photo_url, after_photo_url || null]) ;

        res.status(201).json({
            success: true,
            message: "Report created successfully & passed to the respected Councillor",
